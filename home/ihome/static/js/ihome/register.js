...
function sendSMSCode() {
    ...
    $.get("/api/v1.0/sms_codes/"+mobile,req_data,function (resp) {
        ...
        //所以ajax会帮助我们把这个json字符串转换为js对象，resp就是转换后的对象
        if(resp.errno == "0"){
            var num = 60
            //表示发送成功
...
$(document).ready(function() {
    ...
    $(".form-register").submit(function(e){
        ...
        var req_json = JSON.stringify(req_data); //将js对象转换成json字符串
        $.ajax({
            url:'/api/v1.0/users',
            type:'post',
            data:req_json,//向服务器端提交的json字符串
            contentType:'application/json',//指定向服务器端的数据类型
            dataType:'json',//服务器端向前端返回的数据类型
            headers:{
